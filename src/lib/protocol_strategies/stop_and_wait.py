...
from lib.segment import Segment, const
...
class StopAndWait(ProtocolStrategy):
    ...
    # Por lo general max retry siempre es None,
    # solo se usa en el cierre de la conexión
    # para evitar el problema de los ejercitos
    def send_data(
        self, segment: Segment, max_retry: Optional[int] = 10
    ) -> Optional[Segment]:
        retries = 0
        while retries < max_retry:
            ...
            try:
                ...
                if (
                    ack_pkt.opcode == const.OP_ACK
                    and ack_pkt.seq_num == segment.seq_num
                ):
    ...
    def receive_data(self, max_retry=10) -> Tuple[int, Optional[bytes]]:
        # acá evito que se muera el receptor
        # (cliente y servidor puede ser receptores)
        self.socket.settimeout(const.TIMEOUT)
        retries = 0
        while retries < max_retry:
            try:
                ...
                seq = segment.seq_num
                ack_segment = Segment(const.OP_ACK, seq, self.wsize, b"")
