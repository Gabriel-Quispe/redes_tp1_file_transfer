...
def _run_download(host, port, dst, nombre, protocolo, results, key):
    result = subprocess.run(
        [SRC, DOWNLOAD_CMD, "-H", host, "-p", str(port), "-d", dst, "-n", nombre, "-r", protocolo],
        capture_output=True, text=True
    )
    results[key] = result
...
def step_dos_uploads(ctx, nombre1, nombre2):
    results = {}
    t1 = threading.Thread(target=_run_upload, args=(HOST, PORT, f"/tmp/{nombre1}", nombre1, "stop_and_wait", results, "c1"))
    t2 = threading.Thread(target=_run_upload, args=(HOST, PORT, f"/tmp/{nombre2}", nombre2, "stop_and_wait", results, "c2"))
    t1.start(); t2.start()
    t1.join(); t2.join()
    ctx["results"] = results
    ctx["nombre1"] = nombre1
...
def step_upload_y_download(ctx, nombre_up, nombre_down):
    results = {}
    t1 = threading.Thread(target=_run_upload, args=(HOST, PORT, f"/tmp/{nombre_up}", nombre_up, "stop_and_wait", results, "upload"))
    t2 = threading.Thread(target=_run_download, args=(HOST, PORT, DESTINO, nombre_down, "stop_and_wait", results, "download"))
    t1.start(); t2.start()
    t1.join(); t2.join()
    ctx["results"] = results
...
@when(parsers.parse('los {n:d} clientes ejecutan upload en paralelo con protocolo "{protocolo}"'))
def step_n_uploads(ctx, n, protocolo):
    results = {}
    threads = [
        threading.Thread(target=_run_upload, args=(HOST, PORT, info["path"], nombre, protocolo, results, nombre))
...
@then(parsers.parse('el archivo "{nombre}" existe en el storage del servidor con contenido "{contenido}"'))
...
@then(parsers.parse('el cliente 2 recibe el archivo "{nombre}" con contenido "{contenido}"'))
...
def step_servidor_disponible(servidor_corriendo):
    ...
    result = subprocess.run(
        [SRC, UPLOAD_CMD, "-H", HOST, "-p", str(PORT), "-s", path, "-n", "check.txt", "-r", "stop_and_wait"],
        capture_output=True, text=True
    )
