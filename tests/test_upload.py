...
from conftest import STORAGE, crear_archivo_texto, crear_archivo_binario

SRC        = sys.executable
UPLOAD_CMD = os.path.abspath(os.path.join(os.path.dirname(__file__), "../src/upload"))
FEATURE    = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "../features/upload.feature")
)
...
@when(
    parsers.parse(
        'ejecuto upload con host "{host}" puerto {port:d} src "{src}" '
...
def step_upload_tiempo(ctx, segundos):
    assert ctx["elapsed"] < segundos, (
        f"Upload tardó {ctx['elapsed']:.1f}s, límite {segundos}s"
    )
...
def step_servidor_error(ctx, codigo):
    ...
    assert codigo in salida, (
        f"Se esperaba código '{codigo}' en la salida:\n{salida}"
    )


@then("no se guarda ningún archivo en el storage")
def step_storage_vacio():
    archivos = [f for f in os.listdir(STORAGE) if not f.startswith(".")]
    assert len(archivos) == 0, f"Se encontraron archivos en storage: {archivos}"
